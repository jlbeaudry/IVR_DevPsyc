...
## Negut et al. (2016) - Not done

They used a mixed factorial design with two between-subject factors: test condition
(VC assessment vs. traditional CPT) and clinical status (children with ADHD vs. 
typically developing children), and one within-subjects factor: test modality 
...
(np^2^ = `r round_tidy(clin_eta$Eta_Sq_partial, 3)` 90% CI [`r round(clin_eta$CI_low, 3)`, 
`r round(clin_eta$CI_high, 3)`]). See Table \ref{tab: negut_table} for statistics 
comparing the children with ADHD (n = `r df_negut$adhd_n[1]`) with the typically-developing
children (n = `r df_negut$td_n[1]`) on errors of commission, errors of omission, 
and total correct responses [**I cannot find the means for the mean response time**]. 
I conducted independent t-tests (I report the t-value and p-value) from using 
the `BSDA` R package [@arnholt_bsda_2017]. I also used the `compute.es` R package 
[@re_computees_2020] to calculate the effect size (Cohen's d) and the 95% confidence 
...
what you want to include. We can't do a directcomparison of means across the 
