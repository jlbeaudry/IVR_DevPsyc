...
**[INCLUDE LINK -- ANONYMISE IT FOR BLIND REVIEW].**

## Interpretation of effect sizes {-}
The effect sizes we report can be interpreted according to the following 
standards [INCLUDE A REFERENCE -- COHEN 1988?]:
  ...
  * Vargha-Delaney's A for Mann-Whitney U tests produces a value between 0 and 1.  
      - When A is exactly 0.5, the two techniques achieve equal performance,  
...
*F* (2,78) = 3.82, *p* < .05, *f* = .43, with a large effect. The only post
...
We used the `BSDA` R package [@R-BSDA] to compute the t-test statistic based on 
the summary statistics reported in the paper, and we used the `compute.es` R package 
[@R-compute.es] to calculate the effect size (Cohen's *d*) and the 95% confidence 
intervals (CI) around this parameter.

Within the distraction+helmet (VR) condition, older children showed significantly 
higher pain tolerance (*M* = 70.08, *SD* = 71.22) than the younger children
 (*M* = 31.74, *SD* = 40.36), *t* = 2.19, *p* = .034, *d* = 0.66 [0.08, 1.24].
...
t-values & p-values from Dahlquist et al. (2010, p. 622). Then, we used the R `effsize` 
package [@R-effsize] to calculate the associated effect sizes (Cohen's *d*) with 
...
Specifically, we conducted independent t-tests using the 
the R `BSDA` package [@R-BSDA] and we used the R `compute.es` package 
[@R-compute.es] to calculate the effect sizes and accompanying 95% confidence 
intervals [reported in square brackets]. 
...
df_hua_final <- df_hua_final %>% 
  relocate (vr_m_sd, .after = scale) %>% 
  relocate (ctrl_m_sd, .after = vr_m_sd) %>% 
...
landscape(knitr::kable(df_hua_final, 
            ...
            add_footnote("Note: IVR group had 33 participants and the control group had 32 participants.", notation = "none")
...
The findings supporting our descriptive summary are availabe on page 402 of 
Jeffs et al. (2014). To compare the procedural pain reported by participants in the 
three groups, they used a linear regression model that adjusted for age, sex, 
state anxiety, opiod analgesic use, treatment length, and pre-procedural pain. From 
this model, they estimated participants' procedural pain scores. 
...
what the error bars in Figure 2 represent. 

Jeffs et al. (2015) did report effect sizes comparing the groups (ranging from 
0.535 to 1.25); however, they did not specify the measure of effect size and, thus,
...
For these measures, we used the R `BSDA` package [@R-BSDA] to calculate the t-values
from the summary statistics and we used the R `compute.es` package [@R-compute.es]
to compute effect sizes (Cohen's *d*) with 95% confidence intervals (in square brackets). 
See Table 
\ref{tab:kipping_table} for the summary and inferential statistics. Our calculated 
p-values were slightly different than those reported in the original paper (for all 
but one analyses: nurses' pain ratings during removal), but the 
...
Using the base R stats package [@R-base], we tried to reproduce this analysis, but our 
...
the R `pwr` package [@R-pwr], we calculated Cohen's h, a measure
...
t-values & p-values from Sil et al. (2012, p. 6). Then, we used the R `effsize` 
package [@R-effsize] to calculate the associated effect sizes (Cohen's *d*) with 
95% confidence intervals for the three comparisons.
...
Diaz-Orueta et al. (2014) was necessary. We include relevant statistical details
about the convergent validity of the AULA Nesporala and Conners CPT in the 
manuscript. The only additional analysis needed was to calculate the 
effect size associated with the Mann-Whitney U analysis. We used the 
Mann-Whitney U values reported on p. 338 to calculate Vargha-Delaney's A 
(using formula 7 from Ruscio, 2008). Unfortunately, we could not calculate 
confidence intervals without the original data. 
...
```{r diaz_table,include=TRUE}

knitr::kable(df_diaz_final, 
             caption = "Diaz-Orueta et al. (2014) Paper", 
              align = 'lcccccccccc') %>% 
     kable_styling(full_width = F, 
                latex_options = "HOLD_position")
...
the smallest and largest effect sizes between the AUAL scores comparing
...
reponded faster, `r df_diaz_final[3,3]`, than those without treatment, 
`r df_diaz_final[3,5]`, *U* = `r df_diaz_final[3,6]`, *z* = 
`r df_diaz_final[3,7]`, *p* = `r df_diaz_final[3,8]`. The largest effect between 
the AULA scores was for `r df_diaz_final[1,1]`. Children undergoing treatment 
scored lower, `r df_diaz_final[1,3]`, than those without treatment, `r df_diaz_final[1,5]`, 
*U* = `r df_diaz_final[1,6]`, *z* = `r df_diaz_final[1,7]`, 
*p* = `r df_diaz_final[1,8]`.
...
the test--retest reliability of the ClinicaVR in the manuscript.
...
They ran a MANCOVA with age and IQ as covariates.
We report their V, F, and p-values for the MANCOVA results, but we then used
...
We used the `BSDA` R package [@R-BSDA] to conduct post hoc follow-up tests and
the `compute.es` R package [@R-compute.es] to calculate the effect size 
(Cohen's *d*) with  the 95% confidence intervals (CI). See Table
\ref{tab:negut_table} for relevant descriptive and inferential statistics.
...
n~p~^2^ = `r round_tidy(clin_eta$Eta_Sq_partial, 3)`, 90% CI 
[`r round(clin_eta$CI_low, 3)`, `r round(clin_eta$CI_high, 3)`]). 
 COME BACK TO THIS 
 to conduct independent t-tests comparing the children with ADHD  with the
typically-developing children on errors of commission, errors of omission, and total correct responses 
[**I cannot find the means for response time**].
...
df_pol_final <- df_pol_final %>% 
  ...
  relocate (adhd_m_sd, .after = adhd_n) %>% 
...
We cannot do a direct comparison of means across the VR (AULA Nesplora) test and 
the TOVA test because the values are interpreted differently (high scores on VR 
are indicative of deficit; high scores on the TOVA are indicative of good 
...
# I can't do anything with the canonical correlations from their paper because
  # they only repor the significant values.
```
We report two key analyses (with additional information abut effect sizes) that 
supports their conclusions:

1. Rodriguez et al. conducted two MANCOVAs (one for VR & one for TOVA) to 
...
of the tests. We report these analyses here to supplement the information in 
the manuscript, with the n~p~^2^ as the measure of the effect size. 
...
2. Rodriguez et al. also explored whether the dependent variables (omissions, 
...
classified more of the sample. 
  ...
  + The discriminant function for the VR test correctly classified 56.60% of the 
  sample. 
  ...
  the TOVA was not able to discriminate between children with and without 
  ADHD symptoms. 
...
*Solution*:
  ...
  + Using the Mann-Whitney U values reported on p. 517, we calculated 
  the effect size with the Vargha-Delaney's A (using formula 7 from Ruscio, 2008). 
  But, we could not calculate confidence intervals without the original data.

*Action*:

  + With regard to the block comparisons, there is no obvious effect size for 
  that data, so we should just include some text re: the reported stats from the 
  paper. Something along the lines of: 
    + According to the paper, the deteroriation of performance for children 
    with ADHD across the blocks was significant for the Virtual Classroom test, 
    chi-square(df = 4) = 25,299, p <.001, but not for the CPT test, 
    chi-square(df = 5) = 10,145, p = .07. 
...
df_bio_final <- df %>% 
  select (-c(ctrl_m, ctrl_sd, adhd_m, adhd_sd))

# rearrange the columns we are keeping
df_bio_final <- df_bio_final %>% 
  relocate (measure, .after = test) %>% 
  relocate (c(ctrl_n,adhd_n), .after = block) %>% 
  relocate (notes, .after = last_col())
...
write_csv(df_bio_final, here::here("data_output", "bioulac_data_output.csv"))
```
...
Table 2 on page 5 has the key info, but the labels are very jumbled and unclear.

We do not report the results of the TEA-Ch because only children with ABI 
completed the test; thus, we cannot compare their performance against the control
group of children without ABI. 
...
They used univariate ANOVAs to compare the two groups on the various tests. The 
degrees of freedom for the comparisons on the VC should be 1 and 75; the 
degrees of freedom for the comparisons on the CPRS-R:S should be 1 and 73. In 
Table II (p. 5), they report the F-statistics for the comparisons. They 
corrected for multiple comparisons for the four subscales for the CPRS-R:S 
(.05 / 4 = .0125). 
...
df$notes <- "corrected alpha is .0125"
...
df$notes <- "corrected alpha is .0125; they appear to have rounded down; difference not actually significant"
...
df_gil_final <- df_gil_final %>% 
  ...
  relocate (ctrl_n, .after = ctrl_m_sd) %>% 
...
landscape(knitr::kable(df_gil_final, 
             caption = "Gilboa et al. (2015) Paper", 
              align = 'llccccccccl') %>% 
  column_spec(column = 11, width = "2cm") %>% 
   kable_styling(full_width = F, 
                latex_options = "HOLD_position"))
...
Design: B/w subject, but participants completed pre- and post-assessments. 

Measures: 
  + Paola reported Emotional expression & regulation (PEP-3), social interaction 
  (PEP-3), and emotion recognition (Faces test & Eyes test). They also measures
...
d_aff <- cohens_dav(m.1 = df$post_m[1], 
           m.2 = df$post_m[2], 
           sd.1 = df$post_sd[1],
           sd.2 = df$post_sd[2])
...
different, I used the `BSDA` R package [@R-BSDA] to conduct
independent t-tests for the two measures (affectve expressions and social 
interaction) that had significant interactions. 

For affective expressions, the VR group's post-training score was significantly
higher than the control group's post-training score, *t* = `r round(t_aff$statistic,3)`,
*p* = `r round_tidy(t_aff$p.value,4)`, d~av~ = `r round(d_aff, 3)`. 

For social reciprocity, the VR group's post-training score was also significantly
higher than the control group's post-training score, *t* = `r round(t_soc$statistic,3)`,
*p* = `r round_tidy(t_soc$p.value,4)`, d~av~ = 
`r round(d_soc, 3)`. 
...
df_ip_final <- df_ip %>% 
  select (-c(pre_m, pre_sd, post_m, post_sd, notes))
...
landscape(knitr::kable(df_ip_final, 
             caption = "Ip et al. (2018) Paper", 
              align = 'llcccccccc') %>% 
   kable_styling(full_width = F, 
                latex_options = "HOLD_position"))
