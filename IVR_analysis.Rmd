...
the `BSDA` R package (include REF). I also used the `compute.es` R package 
(include REF) to calculate the effect size (Cohen's d) and the 95% confidence 
...
in Hua section). For these analyses, as in Hua, we used the R packages `BSDA` (REF) and 
`compute.es` (REF).
...
the R `pwr` package (REF), we calculated Cohen's h, a measure
...
(REF) to calculate the effect size (Cohen's d) with confidence intervals for 
...
I have used the R package `effsize` (REF) to calculate the effect size 
