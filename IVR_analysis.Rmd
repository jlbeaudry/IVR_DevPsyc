...
need to figure out if I can calculate it from the summary stats. I imagine
I can't precisely because these are non-parametric tests, so we need the data
to actually calculate it. Possible approaches: 
...
the effect size with the Vagrha-Delaney's A (using formula 7 from
@ruscio_probability-based_2008).

```{r bioulac_VR_play}

# formula for A (from Ruscio & Li papers)
