...
p-values for the MANCOVA results, but we then used
...
We also used the `BSDA` R package [@R-BSDA] to recalculate the post hoc follow-up 
tests and the `compute.es` R package [@R-compute.es] to compute the effect size 
(Cohen's *d*) with 95% confidence intervals (CI). See Table
\ref{tab:negut_table} for relevant descriptive and inferential statistics for the
post hoc tests for commission errors, omission errors, and total correct 
responses. We do not report the post hoc test for the response time variable 
...
We also used the `BSDA` R package [@R-BSDA] to compute the post hoc follow-up 
tests and the `compute.es` R package [@R-compute.es] to compute the effect size 
