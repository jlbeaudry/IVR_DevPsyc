...
although this p-value still exceeds the alpha level of .05. Cohen's h is a measure
of effect size for proportions that can be interpreted using the same rules of 
thumb as Cohen's d for small, medium, and large differences (Cohen, 1988). The 
effect size for this difference was medium-large (h = `r h[1]` [`r h[2]`, 
`r h[3]`]), but with a large 95% confidence interval. Taken together, these 
...
They used a one-way within-subjects ANOVA to compare pain tolerance across the
three conditions (baseline, traditional distraction, VR distrancion). Thye 
reported a large effect size (f) of .56, but they didn't report Cohen's d 
for the mean comparisons nor include the confidence interval around the f value.

I won't be able to recreate their analyses from the summary data, so we should 
just report the t-values & p-values from the paper (p. 6). But, I can calculate
the effect sizes with confidence intervals for the three comparisons.



```{r sil_pain_attempt_1, eval = FALSE}

# I can delete this

df1 <- df_sil

# put it into a useable format

df2 <- df %>% 
  select(-c(measure,scale,comparison, original_p)) %>% 
...
# RUN T-Test
# x is traditional group; mu = baseline


t <- tsum.test(mean.x = df2$mean_traditional, s.x = df2$sd_traditional, 
               n.x = df2$n, 
         mu = df2$mean_baseline,
          alternative = "two.sided", var.equal = TRUE)
