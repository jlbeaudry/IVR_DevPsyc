...
paper. We calculated Vargha-Delaney's A (using formula 7 from Ruscio, 2008) and 
report these values in Table \ref{tab:diaz_table}. Unfortunately, we could not 
calculate confidence intervals without the raw data. 
...
reponded faster, *M* = `r df_diaz_final[3,3]`, than those without treatment, 
...
scored lower, *M* = `r df_diaz_final[1,3]`, than those without treatment,
*M* = `r df_diaz_final[1,5]`, *U* = `r df_diaz_final[1,6]`, 
*z* = `r df_diaz_final[1,7]`, *p* = `r df_diaz_final[1,8]`.
...
landscape(knitr::kable(df_gil_final, 
  ...
  column_spec(column = 8, width = "2cm") %>% 
   kable_styling(full_width = F, 
                latex_options = "HOLD_position")) %>% 
  add_footnote("Note: For the VC test, ABI group: *n* = 41 and control group: *n* = 35. One participant in each group did not complete the CPRS-R:S test. As such, the error degrees of freedom was 75 for the VC measures and 73 for the CPRS-R:S measures. Corrected alpha is .0125.", 
...
In Table \ref{tab:ip_table}, we report the summary statistics and the d-values,
t-values, and p-values reported in Table 3 of Ip et al. (2018) comparing the 
pre- and post-training scores within each group. 
...
*p* = `r round_tidy(t_aff$p.value,4)`, *d* = `r round(d_aff$d, 3)` [`r d_aff$l.d`, 
`r d_aff$u.d`]. For social reciprocity, the VR group's post-training score was 
also significantly higher than the control group's post-training score, *t* = 
`r round(t_soc$statistic,3)`, *p* = `r round_tidy(t_soc$p.value,4)`, *d* = 
`r round(d_soc$d, 3)` [`r d_soc$l.d`, `r d_soc$u.d`].  
...
The summary statistics that were presented in the paper were not statistically
analysed, as far as we could tell. As such, we used the R package {BSDA} [@R-BSDA] 
to conduct independent t-tests comparing the two groups and the R package 
{compute.es} [@R-compute.es] to calculate the effect sizes (Cohen's *d*) and 
ccompanying 95% confidence intervals [reported in square brackets]. 
...
knitr::kable(df_lor_final, 
            ...
            add_footnote("Note: Both groups had 20 participants", threeparttable = TRUE, notation = "none") %>% 
  landscape()
