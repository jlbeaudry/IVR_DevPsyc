...
[[originally i said i used effsize (which I use for within subjects data, 
but the code is actually effectsize....come back to this!]]
...
df_bind$`VR Distraction` <- paste0(df_bind$vr_m, 
                          " (", 
                          df_bind$vr_sd, 
                          ")")
df_bind$`Standard Distraction` <- paste0(df_bind$ctrl_m, 
                            " (", 
                            df_bind$ctrl_sd, 
                            ")")
...
df_hua_final <- df_hua_final %>% 
  relocate (`VR Distraction`, .after = scale) %>% 
  relocate (`Standard Distraction`, .after = `VR Distraction`) %>% 
...
landscape(knitr::kable(df_hua_final, 
            ...
            add_footnote("Note: IVR group had 33 participants and the control group had 32 participants; we report means (with standard deviations).", threeparttable = TRUE, notation = "none")
...
reported in the original paper (for all but one analyses: nurses' pain ratings 
...
compute an effect size measure for this comparison. As such, we provide only 
the summary statistics reported in the original paper (see Table 
\ref{tab:bioulac_table_1}). 
...
reported in the original paper and our computed A values. 
...
knitr::kable(df_bio_final_1, 
     ...
     kable_styling(full_width = F, 
                latex_options = "HOLD_position") %>% 
  add_footnote("Note: Control: *n* = 16; ADHD: *n* = 20", notation = "none")
...
measures from the VC and CPRS-R:S tests. We used report their F values for the 
ANOVA results (from p. 5), but we used the base R stats package [@R-base] to recalculate 
the p-values and the R package {effectsize} [@R-effectsize] to calculate Cohen's
*d* with 95% confidence intervals. We followed their decision to corect
for multiple comparisons (.05 / 4 = .0125). 
...
# change the direction of the d-values for this one analysis
df_gil1$d <- abs(df$d)
...
landscape(knitr::kable(df_gil_final, 
  ...
  column_spec(column = 8, width = "2cm") %>% 
   kable_styling(full_width = F, 
                latex_options = "HOLD_position")) %>% 
  add_footnote("Note: For the VC test, ABI *n* = 41 and controls *n* = 35. One participant in each group did not complete the CPRS-R:S test. As such, the error degrees of freedom was 75 for the VC measures and 73 for the CPRS-R:S measures. Corrected alpha is .0125.", 
...
different, I used the R package {BSDA}  [@R-BSDA] to conduct
